...
import fr.neocle.litebansweb.commands.SubCommands.Bukkit.Reload;
import fr.neocle.litebansweb.commands.SubCommands.Bukkit.Verify;
...
public class BaseCommandBukkit implements CommandExecutor, TabCompleter {
    ...
    public BaseCommandBukkit(Path dataFolder, AuthenticationHandler oauth2Handler, IndexHandler indexHandler, DatabaseUtils databaseUtils, Logger logger) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            sender.sendMessage(ChatColor.YELLOW + "Available subcommands: " + String.join(", ", subCommands.keySet()));
            return true;
        }
