package fr.neocle.flexbans.bukkit.command.subcommand.PlayersWhitelistSubArgs;
...
public class RemovePlayer implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("flexbans.players.remove")) {
            sender.sendMessage(LanguageManager.getMessageComponent("commands.no-permission"));
            return true;
        }

        if (args.length != 2) {
            sender.sendMessage(LanguageManager.getMessageComponent("commands.players-whitelist.remove-player.usage"));
            return true;
        }
