...
public class ModeratorHistoryHandler extends AbstractHandler {
    ...
    public ModeratorHistoryHandler(UsernameUUIDConverters usernameUUIDConverters, PlayerHeadImage playerHeadImage,
                                   DatabaseUtils databaseUtils, NotFoundError notFoundError) {
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        ModeratorRequest moderatorRequest = parseModeratorRequest(target, request);
        if (moderatorRequest == null) return;

        String htmlTemplate = loadTemplate(response);
        if (htmlTemplate == null) return;

        ModeratorData data = fetchModeratorData(moderatorRequest);
        if (data == null) {
        ...
        String pageContent = buildPageContent(htmlTemplate, moderatorRequest, data);
    ...
    private boolean isValidTarget(String target) {
        return target.startsWith("/moderator/");
    }
    ...
    private ModeratorRequest parseModeratorRequest(String target, HttpServletRequest request) throws IOException {
        String moderatorIdentifier = extractModeratorIdentifier(target);
        if (moderatorIdentifier.isEmpty()) {
            return null;
        }

        String moderatorUsername = normalizeModerator(moderatorIdentifier);
        int page = parsePageParameter(request.getParameter("page"));
        String player = normalizePlayer(request.getParameter("player"));

        return new ModeratorRequest(moderatorIdentifier, moderatorUsername, page, player);
    }

    private String extractModeratorIdentifier(String target) {
        return target.replace("/moderator/", "").trim();
    }

    private String normalizeModerator(String moderatorIdentifier) {
        if (moderatorIdentifier.equalsIgnoreCase("[console]")) {
            return "Console";
        }
        if (isUUID(moderatorIdentifier)) {
            return usernameUUIDConverters.UUIDtoUsername(moderatorIdentifier);
        }
    ...
    private String loadTemplate(HttpServletResponse response) throws IOException {
        ...
        if (htmlTemplate == null) {
            FlexLogger.warn("Unable to load HTML template for moderator history page.");
            response.getWriter().write("Error: Unable to load HTML template.");
        }
        return htmlTemplate;
    }

    private ModeratorData fetchModeratorData(ModeratorRequest request) {
        try {
            int totalPages = getTotalPages(request.moderatorUsername, request.player);
            ...
            if (totalPages > 0) {
                fetchAndAddPunishments(punishmentRows, request.moderatorUsername, request.player, offset);
            }

            return new ModeratorData(punishmentRows, Math.max(totalPages, 1), adjustedPage, totalRecords);
    ...
    private String buildPageContent(String htmlTemplate, ModeratorRequest request, ModeratorData data) {
        ServerConfig serverConfig = loadServerConfig();
        UIComponents components = buildUIComponents(data, request);

        String moderatorDescription = String.format(
                "Check all the punishments of %s here. So far, they executed a total of %d punishments.",
                TextUtils.capitalize(request.moderatorIdentifier),
                data.totalRecords
        );
        ...
        String emptyStateContent = hasResults ? "" : buildEmptyStateSection(request.moderatorIdentifier);
        ...
        try {
            return htmlTemplate
                    ...
                    .replace("{{moderator_username}}", TextUtils.capitalize(request.moderatorIdentifier))
                    .replace("{{current_page}}", String.valueOf(data.currentPage))
                    .replace("{{total_pages}}", String.valueOf(data.totalPages))
                    .replace("{{favicon}}", playerHeadImage.getPlayerHeadUrl(request.moderatorIdentifier, "32"))
                    .replace("{{moderator_description}}", moderatorDescription)
                    .replace("{{moderator_icon}}", playerHeadImage.getPlayerHeadUrl(request.moderatorIdentifier, "512"))
                    .replace("{{server_color}}", serverConfig.color)
                    .replace("{{server_color_hover}}", serverConfig.colorDarker);
        } catch (Exception e) {
            return "Error: Unable to generate moderator history page.";
        }
    }

    private UIComponents buildUIComponents(ModeratorData data, ModeratorRequest request) {
        ServerConfig serverConfig = loadServerConfig();

        String backToIndexButton = createBackToIndexButton(serverConfig.color, serverConfig.colorDarker);

        String searchPlayerSection = data.totalRecords > 0
                ? createSearchPlayerSection()
                : "";

        return new UIComponents(backToIndexButton, searchPlayerSection);
    }
    ...
    private String createSearchPlayerSection() {
        return """
                <div class="search mb-2 mt-5 flex items-center">
                    <div class="relative w-full">
                        ...
                        <i class="fa-solid fa-user absolute left-4 top-1/2 transform -translate-y-1/2 text-[#333333] dark:text-[#e0e0e0]"></i>
                    </div>
                </div>
                """;
    }
    ...
    private String buildEmptyStateSection(String moderatorName) {
        return String.format("""
                <div class="flex flex-col items-center justify-center py-16 px-4">
                    <div class="empty-state-container text-center">
                        <i class="fas fa-gavel empty-state-icon text-8xl text-gray-400 dark:text-gray-600 mb-6"></i>
                        <h3 class="text-2xl font-bold text-gray-600 dark:text-gray-400 mb-3">No Punishments Found</h3>
                        <p class="text-lg text-gray-500 dark:text-gray-500 mb-4">%s has not executed any punishments in the database.</p>
                ...
                """, TextUtils.capitalize(moderatorName));
    }
    ...
    private int getTotalPages(String executor, String player) throws SQLException {
        ...
        try (PreparedStatement stmt = prepareStatement(query)) {
            setModeratorParameters(stmt, executor, player);
    ...
    private PreparedStatement prepareStatement(String query) throws SQLException {
        if (usingFlexBans) {
            ...
            FlexLogger.error("No database system is active.");
            throw new SQLException("No database system is active.");
        }
    }

    private void setModeratorParameters(PreparedStatement stmt, String executor, String player) throws SQLException {
        int paramIndex = 1;
    ...
    private record ModeratorRequest(String moderatorIdentifier, String moderatorUsername, int page, String player) {
    }

    private record ModeratorData(StringBuilder punishmentRows, int totalPages, int currentPage, int totalRecords) {
    }

    private record ServerConfig(String color, String colorDarker) {
    }

    private record UIComponents(String backToIndexButton, String searchPlayerSection) {
    }
